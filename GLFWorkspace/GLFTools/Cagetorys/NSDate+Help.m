...
#pragma mark -从NSDate+Helpers复制而来NSCalendarUnitDay
...
- (NSDate *) dateByAddingDaysOnGMT:(NSInteger) days months:(NSInteger) months years:(NSInteger) years
{
    NSDateComponents *dateComponents = [[NSDateComponents alloc] init];
    dateComponents.day = days;
    dateComponents.month = months;
...
{
    return [self dateByAddingDaysOnGMT:days months:0 years:0];
}
...
{
    return [self dateByAddingDaysOnGMT:0 months:months years:0];
}
...
- (NSDate *) monthStartDate
{
    ...
    return [monthStartDate dateByAddingTimeInterval:[[NSTimeZone localTimeZone] secondsFromGMT]];
}
...
{
    //    return round(([self timeIntervalSinceDate:date]-sec) / (60 * 60 * 24));
    NSCalendar *gregorian = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
